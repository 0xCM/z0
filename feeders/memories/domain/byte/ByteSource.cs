...
namespace Z0
{
    ...
    /// </summary>
    public readonly struct ByteSource
    {
        ...
        [MethodImpl(Inline)]
        public static ByteSource Define(string id, ReadOnlySpan<byte> src)
            => new ByteSource(id, src.Length, location(src));

        [MethodImpl(Inline)]
        public static ByteSource Define(string id, int Length, ulong location)
            => new ByteSource(id,Length,location);

        [MethodImpl(Inline)]
        internal ByteSource(string Id, int Length, ulong Location)
        {
            this.Id = Id;
            this.Length = Length;
