...
namespace Z0
{
    ...
    public static class ErrorMessages
    {
        ...
        public static AppMsg KindUnsupported<T>(T kind, string caller, string file, int? line)
            where T : Enum
                => AppMsg.Define($"{typeof(T).Name}.{kind} not supported", SeverityLevel.Error, caller, file, line);
        ...
        public static AppMsg NonGenericMethod(MethodInfo t, string caller, string file, int? line)
            => AppMsg.Define($"The method {t.Name} is nongeneric", SeverityLevel.Error, caller, file, line);

        public static AppMsg GenericMethod(MethodInfo t, string caller, string file, int? line)
            => AppMsg.Define($"The method {t.Name} is generic", SeverityLevel.Error, caller, file, line);
