...
namespace Z0.Logix
{
    ...
    public static partial class VectorizedOps
    {
        ...
        public static Vector256<T> sll<T>(Vector256<T> a, [Imm] byte offset)
            where T : unmanaged
                => ginx.vsll<T>(a, offset);

        [MethodImpl(Inline), VectorOp, PrimalClosures(PrimalKind.UnsignedInts)]
        public static Vector256<T> srl<T>(Vector256<T> a, [Imm] byte offset)
            where T : unmanaged
                => ginx.vsrl<T>(a, offset);

        [MethodImpl(Inline), VectorOp, PrimalClosures(PrimalKind.UnsignedInts)]
        public static Vector256<T> rotl<T>(Vector256<T> a, [Imm] byte offset)
            where T : unmanaged
                => ginx.vrotl<T>(a, offset);

        [MethodImpl(Inline), VectorOp, PrimalClosures(PrimalKind.UnsignedInts)]
        public static Vector256<T> rotr<T>(Vector256<T> a, [Imm] byte offset)
            where T : unmanaged
