...
namespace Z0
{
    ...
    public static class Cpu128OpApi
    {
        ...
        [MethodImpl(Inline)]
        public static Vector128<T> eval<T>(UnaryLogicOpKind kind, Vector128<T> x)
            where T : unmanaged
                => lookup<T>(kind)(x);    

        [MethodImpl(Inline)]
        public static Vector128<T> eval<T>(ShiftOpKind kind, Vector128<T> x, int offset)
            where T : unmanaged
                => lookup<T>(kind)(x,offset);    

        [MethodImpl(Inline)]
        public static Vector128<T> eval<T>(BinaryLogicOpKind kind, Vector128<T> x, Vector128<T> y)
            where T : unmanaged
                => lookup<T>(kind)(x,y);
