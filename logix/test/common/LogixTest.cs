...
namespace Z0.Logix
{
        ...
        where X : LogixTest<X>
    {
        ...
        protected void bm_delegate_bench<T>(BinaryBitwiseOpKind opkind, SystemCounter clock = default)
            where T : unmanaged
        {
            var opname = $"bm_{opkind.Format()}_{suffix<T>()}_delegate";
        ...
        protected void bm_api_bench<T>(BinaryBitwiseOpKind op, SystemCounter clock = default)
            where T : unmanaged
        {
            var opname = $"bm_{op.Format()}_{suffix<T>()}_api";
       ...
       protected void bitwise_logic_check<T>(BinaryBitwiseOpKind kind)
            where T : unmanaged
        {
            ...
            {   
                ...
                var result4 = CpuOpApi.eval(kind, CpuVector.vbroadcast(n128,a), CpuVector.vbroadcast(n128,b)).ToScalar();
                var result5 = CpuOpApi.eval(kind, CpuVector.vbroadcast(n256,a), CpuVector.vbroadcast(n256,b)).ToScalar();
