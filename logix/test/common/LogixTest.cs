...
namespace Z0.Logix
{
        ...
        where X : LogixTest<X>
    {
        ...
        protected void bm_delegate_bench<T>(BinaryBitLogicKind opkind, SystemCounter clock = default)
            where T : unmanaged
        {
            var opname = $"bm_{opkind.Format()}_{Identity.numericid<T>()}_delegate";
        ...
        protected void bm_api_bench<T>(BinaryBitLogicKind op, SystemCounter clock = default)
            where T : unmanaged
        {
            var opname = $"bm_{op.Format()}_{Identity.numericid<T>()}_api";
        ...
        protected void bm_and_bench<T>(SystemCounter clock = default)
            where T : unmanaged
        {
            var opname = $"bm_and_{Identity.numericid<T>()}";
        ...
        protected void bm_xor_bench<T>(SystemCounter clock = default)
            where T : unmanaged
        {
            var opname = $"bm_xor_{Identity.numericid<T>()}";
       ...
       protected void bitwise_logic_check<T>(BinaryBitLogicKind kind)
            where T : unmanaged
        {
            ...
            {   
                ...
                var result4 = VectorOpApi.eval(kind, CpuVector.vbroadcast(n128,a), CpuVector.vbroadcast(n128,b)).ToScalar();
                var result5 = VectorOpApi.eval(kind, CpuVector.vbroadcast(n256,a), CpuVector.vbroadcast(n256,b)).ToScalar();
