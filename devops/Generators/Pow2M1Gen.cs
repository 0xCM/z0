...
namespace Z0
{
    ...
    public class Pow2M1Gen
    {
        ...
        public static byte[] GenData()
            => Pow2M1.All.Select(x => x.vallue).ToSpan().AsBytes().ToArray();

        public static byte[] GenData<T>()
            where T : unmanaged
        {
            
            var values = Pow2M1.Values<T>().Select(x => x.value).ToArray().ToSpan();
            return values.AsBytes().ToArray();
        }

        public static string GenAccessor()
            => InlineData.GenAccessor(GenData(), $"{BasePropName}{Identity.numericid<ulong>()}");

        public static string GenAccessor<T>()
            where T : unmanaged
                => InlineData.GenAccessor(GenData<T>(), $"{BasePropName}{Identity.numericid<T>()}");
