...
namespace Z0.Test
{
    ...
    public class t_vperm4x64 : IntrinsicTest<t_vperm4x64>
    {
        ...
        {
            var x = dinx.vparts(1ul,2,3,4);
            
            var y = dinx.vperm4x64(x, Perm4.ABCD);
            Claim.eq(dinx.vparts(1ul,2,3,4),y);            
            
            y = dinx.vperm4x64(x, Perm4.ABDC);
            Claim.eq(dinx.vparts(1ul,2,4,3), y);

            y = dinx.vperm4x64(x, Perm4.ACBD);
            Claim.eq(dinx.vparts(1ul,3,2,4), y);

            y = dinx.vperm4x64(x, Perm4.ACDB);
            Claim.eq(dinx.vparts(1ul,3,4,2), y);

            y = dinx.vperm4x64(x, Perm4.ADBC);
            Claim.eq(dinx.vparts(1ul,4,2,3), y);

            y = dinx.vperm4x64(x, Perm4.ADCB);
            Claim.eq(dinx.vparts(1ul,4,3,2), y);
