...
namespace Z0.Test
{
    ...
    public class t_vflip : IntrinsicTest<t_vflip>
    {
            ...
            where T : unmanaged
        {
            for(var i=0; i<SampleSize; i++)                        
            {
                ...
                var expect  = Vec128.Load(ref mathspan.not(srcData)[0]);
                var actual = ginx.vnot(in src);
            ...
            where T : unmanaged
        {
            for(var i=0; i<SampleSize; i++)                        
            {
                ...
                var srcData = src.ToSpan();
                var expect  = Vec256.Load(ref mathspan.not(srcData)[0]);
                var actual = ginx.vnot(in src);
                Claim.yea(expect.Equals(actual));
            }
        }
