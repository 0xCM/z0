...
namespace Z0
{
    ...
    partial class ginx
    {
        ...
        static Vec128<T> vxor128f<T>(in Vec128<T> lhs, in Vec128<T> rhs)
            where T : unmanaged
        {
        ...
        static Vec256<T> vxor256f<T>(in Vec256<T> lhs, in Vec256<T> rhs)
            where T : unmanaged
        {
            if(typeof(T) == typeof(float))
                return generic<T>(dfp.vxor(float32(lhs), float32(rhs)));
