    ...
    public static class inxspan
    {
            ...
            where T : unmanaged
        {
            ...
            for(var block = 0; block < blocks; block++)
                ginx.store(ginx.vsub(ginx.vloadu128(in lhs.Block(block)), ginx.vloadu128(in rhs.Block(block))), ref dst.Block(block));
            return dst;
        }
        ...
        {            
            var blocks = dst.BlockCount;
            for(var block = 0; block < blocks; block++)
                ginx.store(ginx.vsub(ginx.vloadu256(in lhs.Block(block)), ginx.vloadu256(in rhs.Block(block))), ref dst.Block(block));
            return dst;
        }
