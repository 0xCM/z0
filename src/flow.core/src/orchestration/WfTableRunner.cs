...
namespace Z0
{
        ...
        where S : unmanaged
    {
        ...
        [MethodImpl(Inline)]
        public WfTableRunner(IWfContext wf, TableMaps<D,S,T,Y> processors, TableSectors<D,S> selectors)
        {
            Wf = wf;
        ...
        [MethodImpl(Inline)]
        public WfTableRunner(IWfContext wf, TableMaps<D,S,T,Y> processors, TableSectors<D,S> selectors, T[] src, Y[] dst)
        {
            Wf = wf;
            Processors = processors;
            Selectors = selectors;
