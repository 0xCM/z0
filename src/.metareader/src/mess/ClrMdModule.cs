...
namespace Z0.MS
{
    ...
    public sealed class ClrmdModule : ClrModule
    {
        ...
        public override MetadataImport? MetadataImport 
            => _metadata ??= _helpers.GetMetadataImport(this);
        ...
        public override IEnumerable<(ulong, int)> EnumerateTypeDefToMethodTableMap() 
            => _helpers.GetSortedTypeDefMap(this);
        ...
        private static int CompareTo((ulong, int) entry, int token) 
            => entry.Item2.CompareTo(token);
    }
}