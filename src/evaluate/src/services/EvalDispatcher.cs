...
namespace Z0
{
    ...
    class EvalDispatcher : IEvalDispatcher
    {
       ...
       void Analyze<T>(in X86ApiMember api, in BinaryEvaluations<T> eval)
        ...
        void Analyze(in Pairs<Fixed8> src, in Triples<Fixed8> dst, in X86ApiMember api)
        {
            for(var i=0; i< 10; i++)
            {
                ref readonly var eval = ref dst[i];
        ...
        bit Dispatch(BufferTokens buffers, in Pairs<Fixed8> src, in X86ApiMember api)
        {
        ...
        bit Dispatch(BufferTokens buffers, in Pairs<Fixed16> src, in X86ApiMember api)
        {
        ...
        Triples<T> Dispatch<E,T>(BufferTokens buffers, in X86ApiMember api, IOpClass<E,T> k, in Pairs<T> src)
            where E : unmanaged, Enum
            where T : unmanaged
        {
