...
namespace Z0.llvm
{
    ...
    public ref struct EtlDatasetQuery
    {
        [MethodImpl(Inline)]
        public static EtlDatasetQuery create(in EtlDatasets src)
            => new EtlDatasetQuery(src);

        readonly EtlDatasets Datasets;

        [MethodImpl(Inline)]
        internal EtlDatasetQuery(in EtlDatasets src)
        {
            Datasets = src;
        }
