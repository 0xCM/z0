...
namespace Z0
{
    ...
    partial class SpannedBits
    {
        ...
        public static void distribute(in byte src, int step, ref uint dst)
            => Vectors.vstore(dvec.vconvert(w64, in skip(src, step*8), w256, w32), ref seek(dst, step*8));

        [MethodImpl(Inline), Op]
        public static void distribute(in byte src, int srcstep, ref uint dst, int dststep)
            => Vectors.vstore(dvec.vconvert(w64, skip(src, srcstep*8), w256, w32), ref seek(dst, dststep*8));
    }
}