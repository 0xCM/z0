...
namespace Z0
{
    ...
    readonly struct InstrinsicsEmitter
    {   
        readonly IAppContext Context;
        
        public static InstrinsicsEmitter create(IAppContext context)
            => new InstrinsicsEmitter(context);

        public InstrinsicsEmitter(IAppContext context)
        {
            Context = context;
        }
        ...
        public void Emit()
        {
            ...
            var list = InstrinsicsDocReader.read();
            var export = Context.AppPaths.ExportRoot;
            var kind = FileExtension.Define("md");
            var folder = InstrinsicsDocReader.folder(kind);
            ...
            for(var i=0; i< list.Length; i++)
            {
                ...
                var fname = filename(current, kind);
                var writer = default(StreamWriter);
                if(!writers.TryGetValue(fname.Name, out writer))
                {   
                    writer = (dir + fname).Writer();
                    var header = text.concat("#", Space, fname.WithoutExtension);
                    writer.WriteLine(header);
                    writers.Add(fname.Name, writer);
                }
