...
namespace Z0
{
    ...
    partial struct Symbols
    {
        ...
        public static Sym16<T> symbol<T>(W16 w, Sym<T> entry)
            where T : unmanaged
                => symbol(w, entry.Identity, (ushort)entry.Index, entry.Name, entry.Kind, entry.Expression);

        public static Sym8<T> symbol<T>(W8 w, Sym<T> entry)
            where T : unmanaged
                => symbol(w, entry.Identity, (byte)entry.Index, entry.Name, entry.Kind, entry.Expression);
    }
}