...
namespace Z0
{
    ...
    partial struct memory
    {
        ...
        public static MemorySeg memseg(MemoryAddress @base, ByteSize bytes)
        ...
        [MethodImpl(Inline), Op]
        public unsafe static MemorySeg memseg(ReadOnlySpan<byte> src)
            => memseg((ulong)gptr(src), src.Length);

        [MethodImpl(Inline), Op]
        public static unsafe MemorySeg memseg(string src)
            => memseg(pchar(src), (uint)src.Length);

        [MethodImpl(Inline), Op]
        public static unsafe MemorySeg memseg(char* src, uint count)
