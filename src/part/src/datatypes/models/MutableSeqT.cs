...
namespace Z0
{
    ...
    public struct MutableSeq<T> : IIndex<T>
    {
        ...
        [MethodImpl(Inline)]
        public bool Equals(MutableSeq<T> rhs)
            => Data.Equals(rhs.Data);

        public MutableSeq<T> Concat(MutableSeq<T> rhs)
            => concat(this, rhs);

        public MutableSeq<Y> Select<Y>(Func<T,Y> selector)
             => mutable(from x in Data select selector(x));

        public MutableSeq<Z> SelectMany<Y,Z>(Func<T,MutableSeq<Y>> lift, Func<T,Y,Z> project)
            => mutable(from x in Data
        ...
        public static MutableSeq<X> concat<X>(MutableSeq<X> head, MutableSeq<X> tail)
            => new MutableSeq<X>(memory.array(head.Storage.Concat(tail.Storage)));
    }
}