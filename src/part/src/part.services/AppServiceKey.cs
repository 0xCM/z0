...
namespace Z0
{
    ...
    /// </summary>
    public readonly struct AppServiceKey
    {
        ...
        [MethodImpl(Inline)]
        public AppServiceKey(uint host, uint contract)
        {
            Host = host;
            Contract = contract;
            KindValue = 0;
        ...
        [MethodImpl(Inline)]
        public AppServiceKey(uint host, uint contract, uint kind, uint tKind)
        {
            Host = host;
            Contract = contract;
            KindValue = kind;
        ...
        public ParseResult<AppServiceKey> Parse(string src)
        {
            var fail = root.unparsed<AppServiceKey>(src);
