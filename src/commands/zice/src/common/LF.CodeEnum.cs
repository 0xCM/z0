...
namespace Z0.Asm.Data
{
	...
	static class CodeEnum 
    {
		...
		static IceEnumValue[] GetValues() =>
			typeof(Code).GetFields().Where(a => a.IsLiteral)
					    .Select(a => new IceEnumValue((uint)(Code)a.GetValue(null)!, a.Name, 
							CommentAttribute.GetDocumentation(a))).ToArray();
		...
		internal static void AddComments(string unitTestDir) 
		{
			...
			{
				var ev = Instance.Values[k];
				if (!string.IsNullOrEmpty(ev.Documentation))
					continue;
				if (!docs.TryGetValue(ev.RawName, out var doc))
					throw new InvalidOperationException($"Couldn't find enum {ev.RawName}");
