...
namespace Z0
{
    ...
    /// <summary>
    /// Represents the value of a type-level septet and thus is an integer in the range [0,31]
    /// </summary>
    public struct septet : IEquatable<analog>
    {
        internal byte data;

        internal const byte MinVal = 0;

        internal const byte MaxVal = 0b1111111;
        ...
        [MethodImpl(Inline)]
        internal septet(byte src)
            => data = (byte)(src & MaxVal);

        [MethodImpl(Inline)]
        internal septet(sbyte src)
            => data = (byte)((uint)src & MaxVal);

        [MethodImpl(Inline)]
        internal septet(short src)
            => data = (byte)((uint)src & MaxVal);

        [MethodImpl(Inline)]
        internal septet(ushort src)
            => data = (byte)(src & MaxVal);

        [MethodImpl(Inline)]    
        internal septet(int x)
            => data = (byte)((uint)x & MaxVal);
        
        [MethodImpl(Inline)]
        internal septet(uint src)
            => data = (byte)(src & MaxVal);

        [MethodImpl(Inline)]
        internal septet(long src)
            => data = (byte)((uint)src & MaxVal);

        [MethodImpl(Inline)]
        internal septet(uint src, bool safe)
            => data = (byte)src;

        [MethodImpl(Inline)]
        internal septet(BK src)
            => data = (byte)src;
