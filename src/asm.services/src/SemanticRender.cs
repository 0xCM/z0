...
namespace Z0.Asm
{
    ...
    public readonly struct SemanticRender
    {
        ...
        public static string format(BranchTargetWidth src)
            => src == 0 ? string.Empty : ((byte)src).ToString();

        [MethodImpl(Inline), Op]
        public static string format(BranchTargetKind src, BranchTargetWidth sz)
        ...
        [MethodImpl(Inline), Op]
        public string Render(MemoryScale src)
        {
            if(src.IsNonEmpty)
