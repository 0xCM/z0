...
namespace Z0
{
    ...
    partial struct AsmRecapture
    {
        ...
        public CapturedAccessor[] Capture(ApiHostUri host, ReadOnlySpan<ResourceAccessor> src, FilePath dst)
        {
            ...
            for(var i=0u; i<count; i++)
            {
                ...
                if(code.IsNonEmpty)
                {
                    ...
                    seek(target, i) = new CapturedAccessor(nameof(AsmRecapture), host, accessor, CreateFunction(data).ValueOrDefault(AsmRoutineCode.Empty), default);
                    Save(data, writer);
                }
            }
        ...
        public CapturedAccessor[] Capture(FilePath src, FolderPath dst)
        {
            ...
            term.print(new CapturedResourceSets(nameof(AsmRecapture), data, src, dst));
            return data;
        }
    }
}