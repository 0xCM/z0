...
namespace Z0
{
    ...
    public sealed class ToolCommand
    {
        ...
        public static ToolCommand Run(string commandLine, ToolOptions options)
        {
            ToolCommand run = new ToolCommand(commandLine, options);
            run.Wait();
            return run;
        }

        public static ToolCommand Run(string commandLine)
        {
            return Run(commandLine, new ToolOptions());
        }
        ...
        /// <param variable="options">Additional qualifiers that control how the process is run</param>
        /// <returns>A Command structure that can be queried to determine ExitCode, Output, etc.</returns>
        public ToolCommand(string commandLine, ToolOptions options)
        {
            Options = options;
        ...
        /// <param variable="commandLine">The command lineNumber to run as a subprocess</param>
        /// </summary>
        public ToolCommand(string commandLine)
            : this(commandLine, new ToolOptions())
        {
        }
