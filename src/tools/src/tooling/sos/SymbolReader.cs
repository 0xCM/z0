    ...
    public class SymbolReader
    {
        ...
        public static KeyGenerator GetKeyGenerator(RuntimeConfiguration config, string moduleFilePath, ulong address, int size, ReadMemoryDelegate readMemory)
        {
            ...
            switch (config)
            {
                ...
                case RuntimeConfiguration.WindowsCore:
                case RuntimeConfiguration.WindowsDesktop:
                    var peFile = new Microsoft.FileFormats.PE.PEFile(new StreamAddressSpace(stream), true);
        ...
        public static bool GetInfoForMethod(string assemblyPath, int methodToken, out MethodDebugInfo debugInfo)
        {
            debugInfo = default;
            try
            {
