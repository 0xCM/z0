...
namespace Z0
{
    ...
    partial struct CodeProjects
    {
        public class ProjectData
        {
            ...
            static RuleSet ResolveRulesetIfAny(MSB.Execution.ProjectInstance projectInstance)
            {
            ...
            static bool FileNameIsNotGenerated(string filePath)
                => !Path.GetFileName(filePath).StartsWith("TemporaryGeneratedFile_", StringComparison.OrdinalIgnoreCase);

            static ImmutableArray<string> GetFullPaths(IEnumerable<MSB.Execution.ProjectItemInstance> items, Func<string, bool> filter = null)
            {
            ...
            static ImmutableArray<PackageReference> GetPackageReferences(ICollection<MSB.Execution.ProjectItemInstance> items)
            {
