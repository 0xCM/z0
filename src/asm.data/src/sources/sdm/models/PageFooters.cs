...
namespace Z0.Asm
{
    partial struct SdmModels
    {
        ...
        public readonly struct PageFooters
        {
            ...
            /// </summary>
            public static PageFooter TitleVolPage() => footer("{Title}", Null, VolPartNumber.Descriptor, ChapterPage.Descriptor);

            /// <summary>
            /// Returns a descripter of the form <paramref parameter='{title}'/> {null} Vol. {Major}.{Minor} {ChapterNumber}.{PageNumber}
            /// </summary>
            public static PageFooter TitleVolPage(string title) => footer(title, Null,VolPartNumber.Descriptor, ChapterPage.Descriptor);

            /// <summary>
            /// Returns a descripter of the form <paramref parameter='{title}'/> {null} <paramref parameter='{vol}'/> {ChapterNumber}.{PageNumber}
            /// </summary>
            public static PageFooter TitleVolPage(string title, VolPartNumber vol) => footer(title, Null, vol, ChapterPage.Descriptor);

            /// <summary>
            /// Returns a descripter of the form <paramref parameter='{title}'/> {null} <paramref parameter='{vol}'/> <paramref parameter='{page}'/>
            /// </summary>
            public static PageFooter TitleVolPage(string title, VolPartNumber vol, ChapterPage page) => footer(title, Null, vol, page);
        }
    }
}