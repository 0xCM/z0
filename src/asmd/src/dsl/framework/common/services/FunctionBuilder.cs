...
{        
    ...
    public readonly ref struct FunctionBuilder
    {
        ...
        internal readonly Span<int> index;
        ...
        public FunctionBuilder(string name, int capacity)
        {
            ...
            this.index = new int[1]{0};
        }

        [MethodImpl(Inline)]
        public FunctionBuilder(asci32[] name, EncodedCommand[] buffer, int[] index)
        {
            this.name = name;
            this.buffer = buffer;
            this.index = index;
        ...
        {
            var f = new function(head(builder.name), builder.buffer.ToArray());
            builder.Index = 0;
