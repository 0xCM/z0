...
namespace Z0
{
    ...
    partial class SRM
    {
        public const string ClrPrefix = "<CLR>";

        public static readonly byte[] WinRTPrefix = new[] {
            (byte)'<',
            (byte)'W',
            (byte)'i',
            (byte)'n',
        ...
        public struct StringHeap: ISrmHeap<StringHeap>
        {
            ...
            {
                [MethodImpl(Inline)]
                get => CliHeapKind.String;
            }

            [MethodImpl(Inline)]
            public StringHeap(MemoryBlock block)
            {
                _lazyVirtualHeap = null;
                Block = TrimEnd(block);
            }

            public StringHeap(MemoryBlock block, MetadataKind metadataKind)
            {
                _lazyVirtualHeap = null;
            ...
            public static string GetVirtualString(StringHandle.VirtualIndex index)
            {
            ...
            string GetVirtualHandleString(StringHandle handle, MetadataStringDecoder utf8Decoder)
            {
                ...
                return handle.StringKind switch
                {
                    ...
                    StringKind.WinRTPrefixed => GetNonVirtualString(handle, utf8Decoder, WinRTPrefix),
                ...
                lock (heap)
                {
                    if (!heap.TryGetMemoryBlock(handle.RawValue, out var block))
                    {
                        byte[] bytes = handle.StringKind switch
                        {
                            ...
                            StringKind.WinRTPrefixed => GetNonVirtualStringBytes(handle, WinRTPrefix),
                            _ => throw ExceptionUtilities.UnexpectedValue(handle.StringKind),
                        };
