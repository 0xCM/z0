...
namespace Z0
{
    ...
    partial struct CilTables
    {
        ...
        public ref partial struct SpecBuilder
        {
            ...
            [Op]
            public static SpecBuilder create(ushort? capacity = null)
                => new SpecBuilder(capacity ?? 20);
            ...
            [MethodImpl(Inline),Op]
            public SpecBuilder(uint capacity)
            {
                Index = 0;
            ...
            [MethodImpl(Inline),Op]
            public SpecBuilder WithField(in FieldSpec src)
            {
            ...
            [MethodImpl(Inline),Op]
            public SpecBuilder WithField(PropertyInfo src)
                => WithField(new FieldSpec(src, src.PropertyType, Index));

            [MethodImpl(Inline),Op]
            public SpecBuilder WithField(MemberName name, Type type)
                => WithField(new FieldSpec(name, type, Index));

            [MethodImpl(Inline), Op]
            public SpecBuilder WithField(ClrField src)
                =>  WithField(new FieldSpec(src.Name, src.FieldType.Name, Index));

            [MethodImpl(Inline), Op]
            public SpecBuilder WithFields(params PropertyInfo[] src)
            {
            ...
            [MethodImpl(Inline), Op]
            public SpecBuilder WithFields(params FieldInfo[] src)
            {
                foreach(var item in src)
                    WithField(item);
                return this;
