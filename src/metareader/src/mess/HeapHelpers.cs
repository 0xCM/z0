...
namespace Z0.MS
{
        ...
        IModuleHelpers, IMethodHelpers, IClrObjectHelpers, IFieldHelpers, IAppDomainHelpers, IThreadHelpers, IExceptionHelpers, IHeapHelpers
    {
        ...
        private readonly DacObjectPool<TypeBuilder> _typeBuilders;

        private readonly DacObjectPool<MethodBuilder> _methodBuilders;

        private readonly DacObjectPool<FieldBuilder> _fieldBuilders;
        ...
        public RuntimeBuilder(ClrInfo clr, DacLibrary library, SOSDac sos)
        {
            ...
            _typeBuilders = new DacObjectPool<TypeBuilder>((owner, obj) => obj.Owner = owner);
            _methodBuilders = new DacObjectPool<MethodBuilder>((owner, obj) => obj.Owner = owner);
            _fieldBuilders = new DacObjectPool<FieldBuilder>((owner, obj) => obj.Owner = owner);
        ...
        bool IFieldHelpers.ReadProperties(ClrType type, int fieldToken, out string? name, out FieldAttributes attributes, out ClrSigParser sigParser)
        {
            CheckDisposed();
            ...
            sigParser = new ClrSigParser(fieldSig, sigLen);
            return true;
        }
