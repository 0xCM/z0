...
namespace Z0
{
    ...
    public readonly struct UriHexQuery : IUriHexQuery
    {
        ...
        public static bool accepts(IdentifiedCode src, NumericKind match)
        ...
        /// <param name="src">The encoded operation</param>
        [MethodImpl(Inline)]
        public static int arity(IdentifiedCode src)
        ...
        public static OpIndex<IdentifiedCode> index(IEnumerable<IdentifiedCode> src)
        ...
        public static IEnumerable<IdentifiedCode> filter(IEnumerable<IdentifiedCode> src, NumericKind k1, NumericKind k2)
            => from code in src
                let kinds = Identify.numeric(code.OpUri.OpId.TextComponents.Skip(1))
                where kinds.Contains(k1) && kinds.Contains(k2)                
                select code;

        public static IEnumerable<IdentifiedCode> filter(IEnumerable<IdentifiedCode> src, NumericKind kind)
            => from code in src
                where accepts(code, kind)
                select code;
                    
        public static IEnumerable<IdentifiedCode> withArity(IEnumerable<IdentifiedCode> src, int count)
            => from code in src
