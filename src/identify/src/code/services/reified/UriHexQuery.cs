...
namespace Z0
{
    ...
    public readonly struct UriHexQuery : IUriHexQuery
    {
        ...
        public static bool accepts(UriHex src, NumericKind match)
        ...
        /// <param name="src">The encoded operation</param>
        [MethodImpl(Inline)]
        public static int arity(UriHex src)
        ...
        public static OpIndex<UriHex> index(IEnumerable<UriHex> src)
        ...
        public static IEnumerable<UriHex> filter(IEnumerable<UriHex> src, NumericKind k1, NumericKind k2)
            => from code in src
                let kinds = Identify.numeric(code.OpUri.OpId.TextComponents.Skip(1))
                where kinds.Contains(k1) && kinds.Contains(k2)                
                select code;

        public static IEnumerable<UriHex> filter(IEnumerable<UriHex> src, NumericKind kind)
            => from code in src
                where accepts(code, kind)
                select code;
                    
        public static IEnumerable<UriHex> withArity(IEnumerable<UriHex> src, int count)
            => from code in src
