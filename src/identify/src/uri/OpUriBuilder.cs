...
{        
    ...
    {        
        ...
        public static string TestCase<C>(Type host, string root, C t = default)
            where C : unmanaged
                =>  text.concat(Identify.owner(host), PathSep, host.Name, PathSep, root, '_', Identify.numeric(t));

        public static string TestCase<W,C>(Type host, string root, W w = default, C t = default, bool generic = true)
            where W : unmanaged, ITypeWidth
            where C : unmanaged
                => $"{Identify.owner(host)}{host.Name}{PathSep}{Identify.Op(root, w.TypeWidth, NumericKinds.kind<C>(), generic)}";
    }
}