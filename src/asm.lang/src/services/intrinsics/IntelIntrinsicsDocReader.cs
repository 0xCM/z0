...
namespace Z0.Asm
{
    ...
    /// </summary>
    [ApiHost]
    struct IntelIntrinsicsDocReader
    {
        public static FS.FolderName folder(FS.FileExt kind)
            => FS.folder("algorithms");

        static XmlReader readxml(XmlDoc src)
            => XmlReader.Create(new StringReader(src.Content));

        [Op]
        public static Intrinsic[] read(XmlDoc xml)
        {
            const ushort max = 7500;
            var entries = new Intrinsic[max];
            var i = -1;
            using var reader = readxml(xml);
