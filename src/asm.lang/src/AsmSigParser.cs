...
namespace Z0.Asm
{
    ...
    public readonly struct AsmSigParser
    {
        ...
        [Op]
        public static bool parse(string src, out AsmSig dst)
        {
            if(text.nonempty(src))
            {
                if(mnemonic(src, out var monic))
                {
                    ...
                    var operands = i > 0 ? src.Substring(i).Split(OperandDelimiter).Map(operand) : sys.empty<AsmSigOperand>();
        ...
        [Op]
        public static AsmSig parse(string src)
        {
        ...
        public static AsmSigOperand operand(string src)
