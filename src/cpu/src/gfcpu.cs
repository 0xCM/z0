...
namespace Z0
{
    ...
    public readonly struct gfcpu
    {
       ...
       public static Vector256<T> vsub<T>(Vector256<T> x, Vector256<T> y)
        ...
        {
            if(typeof(T) == typeof(float))
                return generic<T>(cpu.vhadd(v32f(x), v32f(y)));
            else if(typeof(T) == typeof(double))
                return generic<T>(cpu.vhadd(v64f(x), v64f(y)));
            else
                throw no<T>();
        }
        ...
        {
            if(typeof(T) == typeof(float))
                return generic<T>(cpu.vdiv(v32f(x), v32f(y)));
            else if(typeof(T) == typeof(double))
                return generic<T>(cpu.vdiv(v64f(x), v64f(y)));
            else
                throw no<T>();
        }
