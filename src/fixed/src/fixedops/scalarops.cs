...
namespace Z0
{
    ...
    partial class FixedOps
    {
        ...
        public static UnaryOp1 fix(UnaryOp<bit> f) 
            => a => f(a);

        [MethodImpl(Inline), Op]
        public static UnaryOp8 fix(UnaryOp<sbyte> f)
            => a => f((sbyte)a.Content);

        [MethodImpl(Inline), Op]
        public static UnaryOp8 fix(UnaryOp<byte> f) 
            => a => f((byte)a.Content);

        [MethodImpl(Inline), Op]
        public static UnaryOp16 fix(UnaryOp<short> f)
            => a => f((short)a.Content);

        [MethodImpl(Inline), Op]
        public static UnaryOp16 fix(UnaryOp<ushort> f) 
        ...
        [MethodImpl(Inline)]
        internal static UnaryOp<T> create<T>(MethodInfo src, UnaryOpClass<T> k)
            where T : unmanaged
                => Delegates.unary<T>(src);

        [MethodImpl(Inline)]
        internal static BinaryOp<T> create<T>(MethodInfo src, BinaryOpClass<T> K)
            where T : unmanaged
