//-----------------------------------------------------------------------------
// Derivative Work
// Copyright  : Copyright (c) Microsoft.  All Rights Reserved.  Licensed under the Apache License, Version 2.0.
// Copyright  : (c) Chris Moore, 2020
// License    :  Apache
// Source     : https://github.com/dotnet/symreader-converter/blob/1fe94327d1bdc2ab617c07142ecac869dad66125/src/Microsoft.DiaSymReader.Converter/Utilities/Hash.cs
//-----------------------------------------------------------------------------
namespace Z0
{
    using System;
    ...
    public readonly struct FnvHash
    {
        ...
        {
            int hashCode = FnvHash.FnvOffsetBias;

            for (int i = 0; i < data.Length; i++)
            {
                hashCode = unchecked((hashCode ^ data[i]) * FnvHash.FnvPrime);
            }
        ...
        public static unsafe int GetFNVHashCode(byte* data, int length, out bool isAscii)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                byte b = data[i];
                asciiMask |= b;
                hashCode = unchecked((hashCode ^ b) * FnvHash.FnvPrime);
            }
        ...
        public static int GetFNVHashCode(string text, int start, int length)
        {
            ...
            {
                hashCode = unchecked((hashCode ^ text[i]) * FnvHash.FnvPrime);
            }
        ...
        {
            return CombineFNVHash(FnvHash.FnvOffsetBias, text);
        }
        ...
        {
            return FnvHash.CombineFNVHash(FnvHash.FnvOffsetBias, ch);
        }
        ...
        public static int CombineFNVHash(int hashCode, string text)
        {
            ...
            {
                hashCode = unchecked((hashCode ^ ch) * FnvHash.FnvPrime);
            }
        ...
        {
            return unchecked((hashCode ^ ch) * FnvHash.FnvPrime);
        }
    }
}