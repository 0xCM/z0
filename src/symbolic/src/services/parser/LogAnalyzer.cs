...
namespace Z0
{
    ...
    {
        public static Index<CompilerInvocationInfo> parse(FS.FilePath src)
            => LogAnalyzer.GetInvocations(src.Name).Array().Select(x => x.Describe());
        ...
        public class LogAnalyzer
        {
            ...
            IEnumerable<CompilerInvocation> ProcessLogFileLines(string logFile)
            {
            ...
            bool ProcessInvocation(string line, Action<CompilerInvocation> collector)
            {
                bool csc = false;
                bool vbc = false;
                bool tsc = false;
            ...
            void AddTypeScriptInvocation(string line, Action<CompilerInvocation> collector)
            {
                var invocation = CompilerInvocation.CreateTypeScript(line);
                collector(invocation);
            }

            static void AddInvocation(string line, Action<CompilerInvocation> collector)
            {
            ...
            void AssignProjectFilePath(CompilerInvocation invocation)
            {
