...
namespace Z0
{
        ...
        where T : unmanaged
    {
        ...
        readonly Index<BitfieldSection<T>> _Sections;

        [MethodImpl(Inline)]
        public BitFieldModel(StringAddress name, Index<BitfieldSection<T>> sections)
        {
            Name = name;
        ...
        public ref BitfieldSection<T> this[uint i]
        {
            [MethodImpl(Inline)]
            get => ref _Sections[i];
        }

        public Span<BitfieldSection<T>> Sections
        {
            [MethodImpl(Inline)]
            get => _Sections.Edit;
        }
        ...
        [MethodImpl(Inline)]
        public ref BitfieldSection<T> Section(int index)
            => ref _Sections[index];
    }
}