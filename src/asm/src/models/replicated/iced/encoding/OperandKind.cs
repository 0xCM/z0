...
namespace Z0.Asm.Encoding 
{
    ...
    public readonly struct EnumType
    {
        ...
        public IceEnumValue[] Values {get;}

		public IceEnumValue this[string index] => Values.Where(x => x.RawName == index).FirstOrDefault();

        public EnumType(TypeId id, string docs, IceEnumValue[] values, EnumTypeFlags flags)
        {
            this.TypeId = id;
            Documentation = docs;
            Values = values;
	...
	public enum EnumTypeFlags : uint 
    {
		None			= 0,
		Public			= 0x00000001,
