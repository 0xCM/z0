...
namespace Z0.Asm
{
    ...
    {
        public static string format(in AsmOpCodeTable src, char delimiter = FieldDelimiter)
            => format(src, Formatters.dataset<F>(delimiter)).ToString();

        [Op]
        public static ref readonly DatasetFormatter<F> format(in AsmOpCodeTable src, in DatasetFormatter<F> dst)
        {
