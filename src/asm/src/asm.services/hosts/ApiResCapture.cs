...
namespace Z0
{
    ...
    public readonly struct ApiResCapture
    {
        [Op]
        ...
        public static CapturedApiRes[] capture(IWfShell wf, IAsmContext context, ApiHostUri host, FS.FilePath dst)
            => capture(wf, context, host, Resources.accessors(context.ContextRoot.ApiGlobal.PartComponents), dst);

        public static CapturedApiRes[] capture(IWfShell wf, IAsmContext context, ApiHostUri host, ReadOnlySpan<ApiResAccessor> src, FS.FilePath dst)
        {
        ...
        public static CapturedApiRes[] capture(IWfShell wf, IAsmContext context, FS.FilePath src, FS.FolderPath dst)
        {
            ...
            for(var i=0u; i<count; i++)
            {
                ...
                results.AddRange(capture(wf, context, host, index.View, path));
            }
