...
namespace Z0.Asm
{
    ...
    public ref struct XedFormSummaryParser
    {
        public static XedFormSummaryParser create(IEventSink sink)
            => new XedFormSummaryParser(sink, alloc<TextLine>(16), 8200);

        readonly Index<XedFormInfo> SummaryTarget;

        readonly EventSignal Wf;

        XedFormSummaryParser(IEventSink sink, Span<TextLine> buffer, ushort count)
        {
            SummaryTarget = alloc<XedFormInfo>(count);
            Wf = EventSignal.create(sink, typeof(XedFormSummaryParser));
        }
