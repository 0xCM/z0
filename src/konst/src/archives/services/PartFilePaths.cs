...
{        
    ...
    public readonly struct PartFilePaths : IPartFileIndex
    {   
        public static IPartFileIndex create(FolderPath root = null, FolderName area = null, FolderName subject = null)
            => new PartFilePaths(new CaptureArchives(root ?? Env.Current.LogDir, area ?? FolderName.Empty, subject ?? FolderName.Empty));        

        readonly IPartCaptureArchive CaptureArchive;                 

        [MethodImpl(Inline)]
        public PartFilePaths(IPartCaptureArchive capture)
        {
