...
namespace Z0
{
    ...
    [ApiHost(ApiNames.DbFiles, true)]
    public readonly struct DbFiles
    {
        [MethodImpl(Inline), Op]
        public static DbOptions options(FS.FolderPath root)
        {
            var dst = new DbOptions();
            dst.Root = root;
            dst.Docs = root + FS.folder("docs");
            dst.Tables = root + FS.folder("tables");
            dst.Sources = root + FS.folder("sources");
            dst.Stage = root + FS.folder("stage");
            return dst;
        }
        ...
        [MethodImpl(Inline), Op]
        public static FS.FolderPath sourceRoot(FS.FolderPath root)
            => root + FS.folder("sources");

        [MethodImpl(Inline), Op]
        public static FS.FolderPath stageRoot(FS.FolderPath root)
            => root + FS.folder("stage");
