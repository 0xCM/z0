...
namespace Z0.Data
{
    ...
    public readonly struct TableArchive : ITableArchive
    {
        ...
        [MethodImpl(Inline)]
        public static TableArchive Service(FolderPath root)
        ...
        public Option<FilePath> Deposit<F,R>(R[] src, FileName name)            
            ...
            where R : ITabular
                => Tabular.Store<F,R>().Save(src, Tabular.Specify<F>(), ArchiveRoot + name);

        public Option<FilePath> Deposit<F,R>(R[] src, FolderName folder, FileName name)
            where F : unmanaged, Enum
            where R : ITabular
                => Tabular.Store<F,R>().Save(src, Tabular.Specify<F>(), (ArchiveRoot + folder) + name);
    }
}