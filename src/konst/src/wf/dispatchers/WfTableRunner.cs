...
namespace Z0
{
    ...
    public readonly struct WfTableRunner<F,T,D,S,Y> : ITableDispatcher<F,T,D,S,Y>
        ...
        where S : unmanaged
    {
        ...
        [MethodImpl(Inline)]
        public WfTableRunner(IWfShell wf, TableMaps<D,S,T,Y> processors, TableSectors<D,S> selectors)
        {
            Wf = wf;
        ...
        [MethodImpl(Inline)]
        public WfTableRunner(IWfShell wf, TableMaps<D,S,T,Y> processors, TableSectors<D,S> selectors, T[] src, Y[] dst)
        {
            Wf = wf;
            Processors = processors;
            Selectors = selectors;
