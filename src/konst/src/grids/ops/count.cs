...
namespace Z0
{
    ...
    partial struct GridCalcs
    {
        ...
        public static uint count<T>(uint rows, uint cols)
            where T : unmanaged
                => count(rows, cols, bitwidth<T>());
        ...
        [MethodImpl(Inline)]
        public static uint count<M,N,T>(M m = default, N n = default, T t = default)
            where M : unmanaged, ITypeNat
            where N : unmanaged, ITypeNat
            where T : unmanaged
                => count((uint)nat64u(m), (uint)nat64u(n), bitwidth<T>());
    }
}