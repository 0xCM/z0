...
namespace Z0
{
    ...
    public readonly struct ApiHexQuery : IApiHexQuery
    {
        public static IApiHexQuery Service => default(ApiHexQuery);
        ...
        public static bool accepts(X86UriHex src, NumericKind match)
            => ApiIdentity.numeric(src.Id.TextComponents.Skip(1)).Contains(match);
        ...
        public static IEnumerable<X86UriHex> filter(IEnumerable<X86UriHex> src, NumericKind k1, NumericKind k2)
            => from code in src
                let kinds = ApiIdentity.numeric(code.OpUri.OpId.TextComponents.Skip(1))
