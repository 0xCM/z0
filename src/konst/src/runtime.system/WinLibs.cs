//-----------------------------------------------------------------------------
// Licensed to the .NET Foundation under one or more agreements.
// The .NET Foundation licenses this file to you under the MIT license.
// See the LICENSE file in the project root for more information.
//-----------------------------------------------------------------------------
namespace Z0
{
    [LiteralProvider]
    public readonly struct WinLibs
    {
        public const string Advapi32 = "advapi32.dll";

        public const string BCrypt = "BCrypt.dll";

        public const string CoreComm_L1_1_1 = "api-ms-win-core-comm-l1-1-1.dll";

        public const string CoreComm_L1_1_2 = "api-ms-win-core-comm-l1-1-2.dll";

        public const string Crypt32 = "crypt32.dll";

        public const string CryptUI = "cryptui.dll";

        public const string Error_L1 = "api-ms-win-core-winrt-error-l1-1-0.dll";

        public const string Gdi32 = "gdi32.dll";

        public const string HttpApi = "httpapi.dll";

        public const string IpHlpApi = "iphlpapi.dll";

        public const string Kernel32 = "kernel32.dll";

        public const string Memory_L1_3 = "api-ms-win-core-memory-l1-1-3.dll";

        public const string Mswsock = "mswsock.dll";

        public const string NCrypt = "ncrypt.dll";

        public const string NtDll = "ntdll.dll";

        public const string Odbc32 = "odbc32.dll";

        public const string Ole32 = "ole32.dll";

        public const string OleAut32 = "oleaut32.dll";

        public const string PerfCounter = "perfcounter.dll";

        public const string ProcessApi = "psapi.dll";

        public const string RoBuffer = "api-ms-win-core-winrt-robuffer-l1-1-0.dll";

        public const string Secur32 = "secur32.dll";

        public const string Shell32 = "shell32.dll";

        public const string SspiCli = "sspicli.dll";

        public const string User32 = "user32.dll";

        public const string Version = "version.dll";

        public const string WebSocket = "websocket.dll";

        public const string WinHttp = "winhttp.dll";

        public const string WinMM = "winmm.dll";

        public const string Wldap32 = "wldap32.dll";

        public const string Ws2_32 = "ws2_32.dll";

        public const string Wtsapi32 = "wtsapi32.dll";

        public const string CompressionNative = "clrcompression.dll";

        public const string CoreWinRT = "api-ms-win-core-winrt-l1-1-0.dll";

        public const string MsQuic = "msquic.dll";

        public const string HostPolicy = "hostpolicy.dll";
    }
}