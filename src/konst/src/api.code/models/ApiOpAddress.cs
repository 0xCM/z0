...
namespace Z0
{
    ...
    public readonly struct ApiOpAddress : IAddress<A,W,T>
    {
        ...
        [MethodImpl(Inline)]
        public ApiOpAddress(ApiArtifactKey uri, MemoryAddress address)
        {
            Uri = uri;
            Location = address;
        }
        ...
        [MethodImpl(Inline)]
        public static implicit operator ApiOpAddress((ApiArtifactKey uri, MemoryAddress address) src)
            => new ApiOpAddress(src.uri, src.address);

        public static ApiOpAddress Empty
            => (ApiArtifactKey.Empty, MemoryAddress.Zero);
    }
}