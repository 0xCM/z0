    ...
    public readonly struct MemoryOffsets : IConstIndex<MemoryOffset>
    {
        ...
        public static MemoryOffset legacy(MemoryAddress @base, MemoryAddress relative)
        {
            ...
            if(offset <= byte.MaxValue)
                return Addressable.offset(@base, (byte)offset);
            else if(offset <= ushort.MaxValue)
                return Addressable.offset(@base, (ushort)offset);
            else if(offset <= uint.MaxValue)
                return Addressable.offset(@base, (uint)offset);
            else
                return Addressable.offset(@base, (ulong)offset);
        }
