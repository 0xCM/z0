...
namespace Z0
{
    ...
    public readonly partial struct SpanReader : IApiHost<SpanReader>
    {
        ...
        public ReadOnlySpan<T> cast<T>(ReadOnlySpan<byte> src)
            where T : struct
                => cover(As.edit<byte,T>(first(src)), (src.Length*1)/size<T>());

        [MethodImpl(Inline), Op, Closures(UnsignedInts)]
        public ReadOnlySpan<T> cast<T>(ReadOnlySpan<ushort> src)
            where T : struct
                => cover(As.edit<ushort,T>(As.first(src)), (src.Length*2)/size<T>());

        [MethodImpl(Inline), Op, Closures(UnsignedInts)]
        public ReadOnlySpan<T> cast<T>(ReadOnlySpan<uint> src)
            where T : struct
                => cover(As.edit<uint,T>(first(src)), (src.Length*4)/size<T>());

        [MethodImpl(Inline), Op, Closures(UnsignedInts)]
        public ReadOnlySpan<T> cast<T>(ReadOnlySpan<ulong> src)
            where T : struct
                => cover(As.edit<ulong,T>(first(src)), (src.Length*8)/size<T>());
    }
}