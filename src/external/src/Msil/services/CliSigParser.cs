...
namespace Z0
{
    ...
    [ApiHost]
    public class CliSigParser
    {
        public static CliSigParser create()
            => new CliSigParser();
        ...
        */
        ...
        const int ELEMENT_TYPE_MVAR = 0x1e; // Generic parameter in a generic method definition,represented as number

        const int ELEMENT_TYPE_CMOD_REQD = 0x1f; // Required modifier : followed by a TypeDef or TypeRef token

        const int ELEMENT_TYPE_CMOD_OPT = 0x20; // Optional modifier : followed by a TypeDef or TypeRef token
        ...
        const int ELEMENT_TYPE_PINNED = 0x45; // Denotes a local variable that points at a pinned object

        const int SIG_METHOD_DEFAULT = 0x0; // default calling convention

        const int SIG_METHOD_C = 0x1;// C calling convention

        const int SIG_METHOD_STDCALL = 0x2;// Stdcall calling convention

        const int SIG_METHOD_THISCALL = 0x3; // thiscall  calling convention

        const int SIG_METHOD_FASTCALL = 0x4; // fastcall calling convention

        const int SIG_METHOD_VARARG = 0x5;// vararg calling convention

        const int SIG_FIELD = 0x6; // encodes a field

        const int SIG_LOCAL_SIG = 0x7; // used for the .locals directive

        const int SIG_PROPERTY = 0x8; // used to encode a property

        const int SIG_GENERIC = 0x10; // used to indicate that the method has one or more generic parameters.

        const int SIG_HASTHIS = 0x20; // used to encode the keyword instance in the calling convention

        const int SIG_EXPLICITTHIS = 0x40; // used to encode the keyword explicit in the calling convention

        const int SIG_INDEX_TYPE_TYPEDEF = 0; // ParseTypeDefOrRefEncoded returns this as the out index type for typedefs

        const int SIG_INDEX_TYPE_TYPEREF = 1; // ParseTypeDefOrRefEncoded returns this as the out index type for typerefs

        const int SIG_INDEX_TYPE_TYPESPEC = 2; // ParseTypeDefOrRefEncoded returns this as the out index type for typespecs
        ...
        protected virtual void NotifyRank(int count) { }

        // number of dimensions with specified sizes followed by the size of each
        protected virtual void NotifyNumSizes(int count) { }

        protected virtual void NotifySize(int count) { }
        ...
        protected virtual void NotifyNumLoBounds(int count) { }

        protected virtual void NotifyLoBound(int count) { }
