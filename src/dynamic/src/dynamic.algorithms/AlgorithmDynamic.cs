...
namespace Z0
{
    ...
    {
        public struct Iteratee : IAction<uint,uint,uint>
        {
            uint A0;
        ...
        [MethodImpl(Inline), Op]
        public static unsafe ActionDelegate action(string id, ReadOnlySpan<byte> f)
            => action(id, Buffers.liberate(f));

        [MethodImpl(Inline), Op]
        public static unsafe ActionDelegate action(string id, MemoryAddress f)
        {
        ...
        [MethodImpl(Inline), Op]
        public static Triple<uint> iter(Iteratee target)
        {
