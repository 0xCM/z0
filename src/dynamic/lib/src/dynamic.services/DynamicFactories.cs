...
namespace Z0
{
    ...
    public readonly struct DynamicFactories : IDynamicFactories
    {
        ...
        [MethodImpl(Inline)]
        public IUnaryOpFactory<T> Factory<T>(UnaryOperatorClass<T> op)
            where T :  unmanaged
            => UnaryOpFactory<T>.Service;

        [MethodImpl(Inline)]
        public IBinaryOpFactory<T> Factory<T>(BinaryOperatorClass<T> op)
            where T :  unmanaged
            => BinaryOpFactory<T>.Service;

        [MethodImpl(Inline)]
        public ITernaryOpFactory<T> Factory<T>(TernaryOperatorClass<T> op)
            where T :  unmanaged
