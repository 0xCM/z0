...
namespace Z0.Image
{
    ...
    public sealed unsafe class PEImage : IDisposable
    {
        ...
        const int ImageDataDirectoryCount = 15;

        const int ComDataDirectory = 14;

        const int DebugDataDirectory = 6;

        readonly bool _virt;
        
        int _offset = 0;
        
        readonly int _peHeaderOffset;

        readonly Lazy<ImageFileHeader> _imageFileHeader;

        readonly Lazy<ImageOptionalHeader> _imageOptionalHeader;

        readonly Lazy<CorHeader> _corHeader;

        readonly Lazy<List<SectionHeader>> _sections;

        readonly Lazy<List<PdbInfo>> _pdbs;

        readonly Lazy<IMAGE_DATA_DIRECTORY[]> _directories;

        private readonly Lazy<ResourceEntry> _resources;

        private IMAGE_DATA_DIRECTORY GetDirectory(int index) 
            => _directories.Value[index];
        
        private int HeaderOffset 
            => _peHeaderOffset + sizeof(uint);
        
        private int OptionalHeaderOffset 
            => HeaderOffset + sizeof(IMAGE_FILE_HEADER);
        
        private int SpecificHeaderOffset 
            => OptionalHeaderOffset + sizeof(IMAGE_OPTIONAL_HEADER_AGNOSTIC);
        
        private int DataDirectoryOffset 
            => SpecificHeaderOffset + (IsPE64 ? 5 * 8 : 6 * 4);
        
        private int ImageDataDirectoryOffset 
            => DataDirectoryOffset + ImageDataDirectoryCount * sizeof(IMAGE_DATA_DIRECTORY);
        ...
        internal int ResourceVirtualAddress 
            => (int)GetDirectory(2).VirtualAddress;
        ...
        /// </summary>
        public bool IsPE64 
            => OptionalHeader != null && OptionalHeader.Magic != 0x010b;

        /// <summary>
        /// Gets a value indicating whether this image is managed. (.NET image)
        /// </summary>
        public bool IsManaged 
            => OptionalHeader != null && OptionalHeader.ComDescriptorDirectory.VirtualAddress != 0;
        ...
        public int IndexFileSize 
            => (int)(OptionalHeader?.SizeOfImage ?? 0);
        ...
        /// </summary>
        public ReadOnlyCollection<PdbInfo> Pdbs 
            => _pdbs.Value.AsReadOnly();
        ...
        /// </summary>
        public PeFileVerion GetFileVersionInfo()
        {
            ...
            try
            {
                ...
                return new PeFileVerion(buffer.AsSpan(0, count));
            }
            finally
            {
        ...
        private bool TryRead<T>(out T result) 
            where T : unmanaged => TryRead(_offset, out result);

        private bool TryRead<T>(int offset, out T t) 
            where T : unmanaged
        {
            t = default;
        ...
        internal T Read<T>(int offset) where T : unmanaged 
            => Read<T>(ref offset);
        ...
        public ImageFileHeader ReadImageFileHeader()
        {
            if (!IsValid)
                return null;
        ...
        public IMAGE_DATA_DIRECTORY[] ReadDataDirectories()
        {
        ...
        public ImageOptionalHeader ReadImageOptionalHeader()
        {
            if (!IsValid)
                return null;
        ...
        public CorHeader ReadCorHeader()
        {
