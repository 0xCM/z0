...
namespace Z0
{
    ...
    {
        [MethodImpl(Inline)]
        public static IndexedSeq<T> From<T>(T[] src)    
    ...
    public readonly struct IndexedSeq<T> : IIndexedSeq<IndexedSeq<T>,T>
    {
        ...
        {
            Data = src == null ? Control.array<T>() : src;
        }

        [MethodImpl(Inline)]
        public IndexedSeq(IEnumerable<T> src)
        {
            Data = src == null ? Control.array<T>() : src.ToArray();
        }

        public int Count { [MethodImpl(Inline)] get => Data.Length; }

        public bool IsEmpty { [MethodImpl(Inline)] get => Data == null || Data.Length == 0; }

        public bool IsNonEmpty { [MethodImpl(Inline)] get => !IsEmpty; }

        public int Length { [MethodImpl(Inline)] get => Data.Length;}

        [MethodImpl(Inline)]
        int IFinite.Count()  => Count;

        public ref T this[int index] { [MethodImpl(Inline)] get => ref Data[index]; }

        [MethodImpl(Inline)]
        public ref T Lookup(int index) => ref this[index];
