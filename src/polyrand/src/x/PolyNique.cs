...
namespace Z0
{
    ...
    partial class XSource
    {
        ...
        public static HashSet<T> Distinct<T>(this IDomainSource source, T[] values, int count)
            => source.Distinct(values.Length, count).Select(i => values[i]).ToHashSet();

        public static HashSet<T> Distinct<T>(this IDomainSource source, T pool, int count)
            where T : unmanaged
        {
        ...
        public static HashSet<T> Distinct<T>(this IDomainSource source, T pool, T count)
            where T : unmanaged
        {
            var src = source.DataStream(default(T), pool);
