...
namespace Z0
{
    ...
    {
        public static DirectApiGroup[] direct(IApiHost host, ImmRefinementKind refinment)
            => from g in ApiCollection.direct(host)
                let immg = ImmGroup(host, g, refinment)
                where !immg.IsEmpty
                select g;

        public static GenericApiMethod[] generic(IApiHost host, ImmRefinementKind refinment) 
