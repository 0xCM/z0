        ...
        void Process(ReadOnlySpan<CultRecord> src)
        {
            ...
            for(var i=0; i<count; i++)
            {
                ...
                if(record.RecordKind == CultRecordKind.Statement)
                    AsmLines.Add(new AsmLine(record.LineNumber, asm.statement(record.Statement), asm.comment(record.Comment)));
                else if(record.RecordKind == CultRecordKind.Label)
                    AsmLines.Add(new AsmLine(record.LineNumber, asm.label(record.Label.Format()), asm.comment(record.Comment)));
