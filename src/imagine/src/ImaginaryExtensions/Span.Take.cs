...
namespace Z0
{
    ...
    public static class SpanTake
    {
        ...
        public static ushort TakeUInt16<T>(this Span<T> src)
            where T : unmanaged        
                => Imagine.first(default(W16),src);

        [MethodImpl(Inline)]
        public static uint TakeUInt32<T>(this ReadOnlySpan<T> src)
            where T : unmanaged
                => Imagine.first(default(W32),src);
        ...
        {
            var storage = 0u;
            ref var dst = ref Imagine.@as<uint,byte>(ref storage);
            ref readonly var src8 = ref Imagine.first(default(W8),src);
            Imagine.seek(ref dst,0) = Imagine.skip(src8,0);
            Imagine.seek(ref dst,1) = Imagine.skip(src8,1);
            Imagine.seek(ref dst,2) = Imagine.skip(src8,2);
            return storage;
        }
