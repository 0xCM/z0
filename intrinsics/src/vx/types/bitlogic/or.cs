...
namespace Z0
{
    ...
    partial class VXTypes
    {
        ...
        public readonly struct Or128<T> : IVBinOp128D<T>, IBinaryBlockedOp128<T>
            where T : unmanaged
        {
            ...
            public ref readonly Block128<T> Invoke(in Block128<T> a, in Block128<T> b, in Block128<T> c)            
                => ref vblocks.vor(a,b,c);

        }

        [PrimalClosures(PrimalKind.Integers)]
        public readonly struct Or256<T> : IVBinOp256D<T>, IBinaryBlockedOp256<T>
            where T : unmanaged
        {
            public const string Name = "vor";
