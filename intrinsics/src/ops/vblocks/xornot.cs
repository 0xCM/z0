...
namespace Z0
{
    ...
    {     
        ...
        [MethodImpl(Inline)]
        public static void xornot<T>(N128 n, int vcount, int blocklen, in T a, in T b, ref T z)
            where T : unmanaged
        {
        ...
        [MethodImpl(Inline)]
        public static void xornot<T>(N256 n, int vcount, int blocklen,  in T a, in T b, ref T z)
            where T : unmanaged
        {
            for(int i=0, offset = 0; i < vcount; i++, offset += blocklen)
                xornot(n, in skip(in a, offset), in skip(in b, offset), ref seek(ref z, offset));
        ...
        [MethodImpl(Inline)]
        public static void xornot<T>(in ConstBlock128<T> xb, in ConstBlock128<T> yb, in Block128<T> zb)
            where T : unmanaged
        {            
            var count = zb.BlockCount;
            for(var block = 0; block < count; block++)
                vstore(ginx.vxornot(xb.LoadVector(block), yb.LoadVector(block)), ref zb.BlockSeek(block));
        }

        [MethodImpl(Inline)]
        public static void xornot<T>(in ConstBlock256<T> xb, in ConstBlock256<T> yb, in Block256<T> zb)
            where T : unmanaged
        {            
            var count = zb.BlockCount;
            for(var block = 0; block < count; block++)
