...
namespace Z0
{
    ...
    {     
        ...
        [MethodImpl(Inline)]
        public static void rotl<T>(N128 n, int vcount, int step, in T a, int shift, ref T z)
            where T : unmanaged
        {
        ...
        [MethodImpl(Inline)]
        public static void rotl<T>(N256 n, int vcount, int step, in T a, int shift, ref T z)
            where T : unmanaged
        {
            for(int i=0, offset = 0; i < vcount; i++, offset += step)
                rotl(n, in skip(in a, offset), shift, ref seek(ref z, offset));
            ...
            where T : unmanaged
        {
            ...
            for(var block = 0; block < count; block++)
                vstore(ginx.vrotl(xb.LoadVector(block), shift), ref zb.BlockSeek(block));
        } 
