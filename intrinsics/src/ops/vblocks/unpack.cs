       ...
       [MethodImpl(Inline)]
            ...
            where T : unmanaged
        {
            ...
            for(var block = 0; block < count; block++)
                vstore(ginx.vmergelo(xb.LoadVector(block), yb.LoadVector(block)), ref zb.BlockRef(block));
        } 
            ...
            where T : unmanaged
        {
            var count = zb.BlockCount;
            for(var block = 0; block < count; block++)
                vstore(ginx.vmergehi(xb.LoadVector(block), yb.LoadVector(block)), ref zb.BlockRef(block));
        } 
