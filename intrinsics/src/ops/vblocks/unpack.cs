       ...
       [MethodImpl(Inline)]
            ...
            where T : unmanaged
        {
            ...
            for(var block = 0; block < count; block++)
                vstore(ginx.vunpacklo(xb.LoadVector(block), yb.LoadVector(block)), ref zb.BlockRef(block));
        } 
            ...
            where T : unmanaged
        {
            var count = zb.BlockCount;
            for(var block = 0; block < count; block++)
                vstore(ginx.vunpackhi(xb.LoadVector(block), yb.LoadVector(block)), ref zb.BlockRef(block));
        } 
