...
namespace Z0
{
    ...
    {     
        ...
        [MethodImpl(Inline)]
        public static void eq<T>(N128 n, int vcount, int blocklen, in T a, in T b, ref T z)
            where T : unmanaged
        {
        ...
        [MethodImpl(Inline)]
        public static void eq<T>(N256 n, int vcount, int blocklen, in T a, in T b, ref T z)
            where T : unmanaged
        {
            for(int i=0, offset = 0; i < vcount; i++, offset += blocklen)
                eq(n, in skip(in a, offset), in skip(in b, offset), ref seek(ref z, offset));
