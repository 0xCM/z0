...
namespace Z0
{
    ...
    {     
        ...
        [MethodImpl(Inline)]
        public static void xorsl<T>(N128 n, int vcount, int blocklen, in T a, byte shift, ref T z)
            where T : unmanaged
        {
        ...
        [MethodImpl(Inline)]
        public static void xorsl<T>(N256 n, int vcount, int blocklen, in T a, byte shift, ref T z)
            where T : unmanaged
        {
            for(int i=0, offset = 0; i < vcount; i++, offset += blocklen)
                xorsl(n, in skip(in a, offset), shift, ref seek(ref z, offset));
            ...
            where T : unmanaged
        {
            ...
            for(var block = 0; block < count; block++)
                vstore(ginx.vxorsl(xb.LoadVector(block), shift), ref zb.BlockRef(block));
        } 
