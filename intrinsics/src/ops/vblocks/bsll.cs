...
namespace Z0
{
    ...
    {     
        ...
        [MethodImpl(Inline)]
        public static void bsll<T>(N128 n, int vcount, int blocklen, in T a, byte shift, ref T z)
            where T : unmanaged
        {
        ...
        [MethodImpl(Inline)]
        public static void bsll<T>(N256 n, int vcount, int blocklen, in T a, byte shift, ref T z)
            where T : unmanaged
        {
            for(int i=0, offset = 0; i < vcount; i++, offset += blocklen)
                bsll(n, in skip(in a, offset), shift, ref seek(ref z, offset));
        ...
        [MethodImpl(Inline)]
        public static void bsll<T>(in ConstBlock128<T> xb, byte shift, in Block128<T> zb)
            where T : unmanaged
        {
        ...
        [MethodImpl(Inline)]
        public static void bsll<T>(in ConstBlock256<T> xb, byte shift, in Block256<T> zb)
            where T : unmanaged
        {
            var count = zb.BlockCount;
            for(var block = 0; block < count; block++)
