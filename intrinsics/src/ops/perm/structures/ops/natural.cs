...
namespace Z0
{
    ...
    partial class Perm
    {
        ...
        public static NatPerm<N4> natural(Perm4 spec)
        {
            uint data = (byte)spec;
            var dst = NatPerm<N4>.Alloc();
            for(int i=0, offset = 0; i<dst.Length; i++, offset +=2)
                dst[i] = (int)BitMask.between(data, offset, (offset + 1));
            return dst;
        }
        ...
        /// </summary>
        /// <param name="spec">The representative</param>
        public static NatPerm<N8> natural(Perm8 spec)
        {
            uint data = (uint)spec;
            var dst = NatPerm<N8>.Alloc();
            for(int i=0, offset = 0; i<dst.Length; i++, offset +=3)
                dst[i] = (int)BitMask.between(data, offset, offset + 2);
            return dst;
        }
        ...
        /// </summary>
        /// <param name="spec">The representative</param>
        public static NatPerm<N16> natural(Perm16 spec)
        {
            ulong data = (ulong)spec;
            var dst = NatPerm<N16>.Alloc();
            for(int i=0, offset = 0; i<dst.Length; i++, offset +=4)
                dst[i] = (int)BitMask.between(data, offset, offset + 3);
            return dst;
        }
