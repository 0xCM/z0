...
namespace Z0.CpuModel
{
    ...
    public struct YMM : IMMReg, ICpuReg256, IEquatable<YMM>
    {
        ...
        {
            return index <= 5 ? VolatilityKind.Volatile : VolatilityKind.Mixed;
        }

        public VolatilityKind Volatility(int index, int part)
        {
            if(part == 0)
            {
                if(index <= 5)
                    return VolatilityKind.Volatile;
                else
                    return VolatilityKind.NonVolatile;
            }
            else if(part == 1)
                return VolatilityKind.Volatile;
            else
