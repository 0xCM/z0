...
namespace Z0
{
    ...
    public class t_hex : t_symbolic<t_hex>
    {
        public void hexdigits_define()
        {
            ...
            Claim.eq(Hex.hex(x0,Pos0), x0c[Pos0]);
            Claim.eq(Hex.hex(x0,Pos1), x0c[1]);
                                
            ushort x1 = 0x4A3C;
            var x1c = Hex.chars(x1);
            Claim.eq(4,x1c.Length);
            Claim.eq(Hex.hex(x1,Pos0), x1c[Pos0]);
            Claim.eq(Hex.hex(x1,1), x1c[1]);
            Claim.eq(Hex.hex(x1,2), x1c[2]);
            Claim.eq(Hex.hex(x1,3), x1c[3]);

            uint x2 = 0x10F74A3C;
            var x2c = Hex.chars(x2);
            Claim.eq(8,x2c.Length);
            Claim.eq(Hex.hex(x2,Pos0), x2c[Pos0]);
            Claim.eq(Hex.hex(x2,1), x2c[1]);
            Claim.eq(Hex.hex(x2,2), x2c[2]);
            Claim.eq(Hex.hex(x2,3), x2c[3]);
            Claim.eq(Hex.hex(x2,4), x2c[4]);
            Claim.eq(Hex.hex(x2,5), x2c[5]);
            Claim.eq(Hex.hex(x2,6), x2c[6]);
            Claim.eq(Hex.hex(x2,7), x2c[7]);

            ulong x3 = 0x3383107810F74A3C;
            var x3c = Hex.chars(x3);
            Claim.eq(16,x3c.Length);
            Claim.eq(Hex.hex(x3,0), x3c[0]);
            Claim.eq(Hex.hex(x3,1), x3c[1]);
            Claim.eq(Hex.hex(x3,2), x3c[2]);
            Claim.eq(Hex.hex(x3,3), x3c[3]);
            Claim.eq(Hex.hex(x3,4), x3c[4]);
            Claim.eq(Hex.hex(x3,5), x3c[5]);
            Claim.eq(Hex.hex(x3,6), x3c[6]);
            Claim.eq(Hex.hex(x3,7), x3c[7]);
            Claim.eq(Hex.hex(x3,8), x3c[8]);
            Claim.eq(Hex.hex(x3,9), x3c[9]);
            Claim.eq(Hex.hex(x3,10), x3c[10]);
            Claim.eq(Hex.hex(x3,11), x3c[11]);
            Claim.eq(Hex.hex(x3,12), x3c[12]);
            Claim.eq(Hex.hex(x3,13), x3c[13]);
            Claim.eq(Hex.hex(x3,14), x3c[14]);
            Claim.eq(Hex.hex(x3,15), x3c[15]);
        }
