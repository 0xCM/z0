...
namespace Z0
{
    ...
    public class t_bitslice : t_bitcore<t_bitslice>
    {
        public void bitslice_32u()
        {
            ...
            var x0 = gbits.extract(src, ScalarCast.uint8(i += 3), 3);
            var bsx0 = x0.ToBitString();
            var y0 = bsx0.TakeScalar<uint>();
            Claim.eq(x0,y0);

            var x1 = gbits.extract(src, ScalarCast.uint8(i += 3), 3);
            var bsx1 = x1.ToBitString();
            var y1 = bsx1.TakeScalar<uint>();
            Claim.eq(x1,y1);

            var x2 = gbits.extract(src, ScalarCast.uint8(i += 3), 3);
            var bsx2 = x2.ToBitString();
            var y2 = bsx2.TakeScalar<uint>();
            Claim.eq(x2,y2);

            var x3 = gbits.extract(src, ScalarCast.uint8(i += 3), 3);
