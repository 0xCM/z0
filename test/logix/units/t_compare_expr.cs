    ...
    public class t_compare_expr : TypedLogixTest<t_compare_expr>
    {
            ...
            where T : unmanaged
        {
            ...
            for(var i=0; i<RepCount; i++)
            {
                ...
                var expect = PredicateApi.eval(BinaryComparisonApiClass.Lt,a,b);
            ...
            where T : unmanaged
        {
            ...
            for(var i=0; i<RepCount; i++)
            {
                ...
                var b = vb.Set(Random);
                var result = eval(x).Value;
                var expect = PredicateApi.eval(BinaryComparisonApiClass.LtEq,a,b);
                Claim.Eq(expect,result);
            }
        }
            ...
            where T : unmanaged
        {
            ...
            for(var i=0; i<RepCount; i++)
            {
                var a = va.Set(Random);
                var b = vb.Set(Random);
                var expect = PredicateApi.eval(BinaryComparisonApiClass.Gt,a,b);
            ...
            where T : unmanaged
        {
            ...
            for(var i=0; i<RepCount; i++)
            {
                var a = va.Set(Random);
                var b = vb.Set(Random);
                var expect = PredicateApi.eval(BinaryComparisonApiClass.GtEq,a,b);
                var actual = eval(x).Value;
                Claim.Eq(expect,actual);
            }
        }
