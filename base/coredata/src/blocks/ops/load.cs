...
namespace Z0
{
    ...
    partial class MemBlocks
    {
        ...
        [MethodImpl(Inline)]
        public static unsafe Block128<T> load<T>(N128 n, void* src, int length)
            where T : unmanaged
        {
            if(!aligned<T>(n,length))
                badsize(n,length);
            
            return new Block128<T>(src,length);
        }

        [MethodImpl(Inline)]
        public static unsafe Block256<T> load<T>(N256 n, void* src, int length)
            where T : unmanaged
        {
            if(!aligned<T>(n,length))
                badsize(n,length);

            return new Block256<T>(src,length);
        }
            ...
            where T : unmanaged
        {
            var bz = blockcount<T>(n, src.Length, out int remainder);
            if(remainder == 0)
                return Block128<T>.TransferUnsafe(src);
            else
            {
                var dst = alloc<T>(n, bz + 1);
                src.CopyTo(dst);
            ...
            where T : unmanaged
        {
            var bz = blockcount<T>(n, src.Length, out int remainder);
            if(remainder == 0)
                return Block256<T>.TransferUnsafe(src);
            else
            {
                var dst = alloc<T>(n, bz + 1);
                src.CopyTo(dst);
