...
namespace Z0
{
    ...
    /// <summary>
    /// Encapsulates a readonly span that can be evenly partitioned into 16-bit blocks
    /// </summary>
    public readonly ref struct ConstBlock16<T>
        where T : unmanaged
    {
        ...
        [MethodImpl(Inline)]
        public static implicit operator ReadOnlySpan<T>(in ConstBlock16<T> src)
            => src.data;

        [MethodImpl(Inline)]
        internal ConstBlock16(ReadOnlySpan<T> src)
            => this.data = src;        
        ...
        [MethodImpl(Inline)]
        public ConstBlock16<S> As<S>()                
            where S : unmanaged
                => new ConstBlock16<S>(MemoryMarshal.Cast<T,S>(data));                    
