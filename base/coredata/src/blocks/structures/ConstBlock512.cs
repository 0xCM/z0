...
namespace Z0
{
    ...
    /// <summary>
    /// Encapsulates a readonly span that can be evenly partitioned into 512-bit blocks
    /// </summary>
    public readonly ref struct ConstBlock512<T>
        where T : unmanaged
    {
        ...
        [MethodImpl(Inline)]
        public static implicit operator ReadOnlySpan<T>(in ConstBlock512<T> xb)
            => xb.data;

        [MethodImpl(Inline)]
        internal ConstBlock512(ReadOnlySpan<T> src)
            => this.data = src;
        ...
        [MethodImpl(Inline)]
        public ReadOnlySpan<T> LoBlock(int block)
        {
        ...
        /// <param name="block">The block-relative index</param>
        [MethodImpl(Inline)]
        public ReadOnlySpan<T> HiBlock(int block)
        {
            var count = BlockLength / 2;
        ...
        [MethodImpl(Inline)]
        public ConstBlock512<S> As<S>()                
            where S : unmanaged
                => new ConstBlock512<S>(MemoryMarshal.Cast<T,S>(data));
