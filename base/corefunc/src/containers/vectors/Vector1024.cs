...
namespace Z0
{
        ...
        where T : unmanaged
    {
        ...
        public string Format(char sep = ',', int pad = 0)
            => bracket(concat(
                    A.Format(sep:AsciSym.Comma, bracketed:false), $"{sep} ",
                    B.Format(sep:AsciSym.Comma,bracketed:false), $"{sep} ",
                    C.Format(sep:AsciSym.Comma,bracketed:false), $"{sep} ",
                    D.Format(sep:AsciSym.Comma,bracketed:false))
                    );
