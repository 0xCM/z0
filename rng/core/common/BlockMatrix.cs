...
namespace Z0
{
    ...
    partial class RngX
    {
        ...
        public static BlockMatrix<M,N,T> BlockMatrix<M,N,T>(this IPolyrand random, Interval<T> domain, M m = default, N n = default)
            ...
            where N : unmanaged, ITypeNat
            where T : unmanaged    
                => Z0.Matrix.blockload<M,N,T>(random.Blocks<T>(n256, DataBlocks.blockcount<M,N,T>(n256), domain));                    
        ...
        /// <typeparam name="T">The element type</typeparam>
         public static BlockMatrix<N,T> BlockMatrix<N,T>(this IPolyrand random, Interval<T>? domain = null)
            where N : unmanaged, ITypeNat
            where T : unmanaged    
                => Z0.Matrix.blockload<N,T>(random.Blocks<T>(n256, DataBlocks.blockcount<M,N,T>(n256), domain));                    
