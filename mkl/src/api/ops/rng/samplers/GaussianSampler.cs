        ...
        where T : unmanaged
    {
        ...
        protected override int FillBuffer(Span<T> buffer)
        {
            if(typeof(T) == typeof(float))
                sample.gaussian(Source, float32(DistSpec.Mean), float32(DistSpec.StdDev), SpanOps.span32f(buffer));
            else if(typeof(T) == typeof(double))
                sample.gaussian(Source, float64(DistSpec.Mean), float64(DistSpec.StdDev), SpanOps.span64f(buffer));
            else 
                throw unsupported<T>();
