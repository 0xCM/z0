...
; static ReadOnlySpan<byte> vreverse_128x8uBytes => new byte[36]{0xC5,0xF8,0x77,0x66,0x90,0xC5,0xF9,0x10,0x02,0x48,0xB8,0x99,0xD0,0x79,0x78,0x35,0x02,0x00,0x00,0xC5,0xFB,0xF0,0x08,0xC4,0xE2,0x79,0x00,0xC1,0xC5,0xF9,0x11,0x01,0x48,0x8B,0xC1,0xC3};
0000h vzeroupper                              ; VZEROUPPER(VEX_Vzeroupper)                 encoding(VEX, 3 bytes) = c5 f8 77
0003h xchg ax,ax                              ; NOP(Nopw)                                  encoding(2 bytes) = 66 90
0005h vmovupd xmm0,[rdx]                      ; VMOVUPD(VEX_Vmovupd_xmm_xmmm128) [XMM0,mem(Packed128_Float64,RDX:br,:sr)] encoding(VEX, 4 bytes) = c5 f9 10 02
0009h mov rax,2357879D099h                    ; MOV(Mov_r64_imm64) [RAX,2357879d099h:imm64] encoding(10 bytes) = 48 b8 99 d0 79 78 35 02 00 00
...
0034h mov rax,2357879CFA9h                    ; MOV(Mov_r64_imm64) [RAX,2357879cfa9h:imm64] encoding(10 bytes) = 48 b8 a9 cf 79 78 35 02 00 00
