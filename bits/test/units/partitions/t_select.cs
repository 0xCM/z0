...
namespace Z0.Test
{
    ...
    public class t_select : ScalarBitTest<t_select>
    {        
        ...
        {            
            ...
            var b = BitParts.select(src, maskB);
            Claim.eq(a,b);
        ...
        public void select_32x4()
        {
            ...
            var bv = src.ToBitVector();
            var x0 = BitParts.select(src,Part32x4.Part0).ToBitVector(n32);
            Claim.eq(bv[0..3], x0);
        ...
        public void select_32x8()
        {
            ...
            var x0 = 0b11110000u;
            var y0 = BitParts.select(src, Part32x8.Part0);
            Claim.eq(x0, y0);

            var y1  = BitParts.select(src, Part32x8.Part1);
            var x1 = 0b11001100u;
            Claim.eq(x1, y1);

            var x2 = 0b01010101u;
            var y2  = BitParts.select(src, Part32x8.Part2);
            Claim.eq(x2, y2);
            
            var x3 = 0b11100111u;
            var y3  = BitParts.select(src, Part32x8.Part3);
            Claim.eq(x3, y3);
        }
