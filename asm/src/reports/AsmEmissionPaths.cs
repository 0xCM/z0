...
{        
    ...
    public readonly struct AsmEmissionPaths
    {
        ...
        public FilePath ExtractReport(ApiHost host)
            => CaptureRoot + FileName.Define(text.concat(host.Owner.Format(), text.dot(), host.HostName, text.dash(), "extract"), Ext.Csv);

        public FilePath LocationReport(ApiHost host)
            => LocationRoot + FileName.Define(text.concat(host.Owner.Format(), text.dash(), host.HostName), Ext.Csv);
        ...
        public FilePath ParseReport(ApiHost host)
            => OwnerRoot(host.Owner) + FileName.Define(text.concat(host.Owner.Format(), text.dot(), host.HostName), Ext.Csv);

        public FilePath DecodedAsmDetail(ApiHost host)
            => OwnerRoot(host.Owner) + FileName.Define(text.concat(host.Owner.Format(), text.dot(), host.HostName), Ext.Asm);
