...
namespace Z0.Asm
{
    ...
    {        
        ...
        public static IEvalDispatcher Create(IAsmContext context, IAppMsgSink sink, IPolyrand random)
            => new EvalDispatcher(context,sink,random);

        EvalDispatcher(IAsmContext context, IAppMsgSink sink, IPolyrand random)
        {
        ...
        MemberEvaluator Evaluator(in BufferSeq buffers)
            => MemberEvaluator.Create(Context, Random, buffers);
       ...
       void Analyze<T>(in ApiMemberCode api, in BinaryEval<T> eval)
        ...
        {        
            ...
            var evaluator = Context.Evaluator(buffers, k);
