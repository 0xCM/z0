...
namespace Z0.Asm
{
    ...
    public readonly ref struct MemberEvaluator
    {
        ...
        [MethodImpl(Inline)]        
        public static MemberEvaluator Create(IContext context, IPolyrand random, in BufferSeq buffers)
            => new MemberEvaluator(context, random, buffers);

        [MethodImpl(Inline)]        
        MemberEvaluator(IContext context, IPolyrand random, in BufferSeq buffers)
        {
        ...
        public ref readonly Triples<T> Eval<T>(in ApiMemberCode api, K.BinaryOpClass op, in Pairs<T> src, in Triples<T> dst)
            where T : unmanaged
        {
            var count = src.Count;
        ...
        /// <param name="src">The source pairs over which to evaluate the operator</param>
        /// <typeparam name="T">The operand type</typeparam>
        public Triples<T> Eval<T>(in ApiMemberCode api, K.BinaryOpClass op, in Pairs<T> src)
            where T : unmanaged
        ...
        /// <param name="src">The source pairs over which to evaluate the operator</param>
        /// <typeparam name="T">The operand type</typeparam>
        public Triples<F> EvalFixed<F>(in ApiMemberCode api, K.BinaryOpClass op, in Pairs<F> src)
