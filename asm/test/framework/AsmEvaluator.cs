...
namespace Z0.Asm.Validation
{
    ...
    public readonly ref struct AsmEvaluator
    {
        ...
        public Points<N3,T> EvalOperator<T>(in ApiMemberCode api, Points<N2,T> src)
            where T : unmanaged
        {
            var count = src.Count;
            var f = buffers[Left].EmitBinaryOp<T>(api);
            var dst = Points.alloc<N3,T>(src.Count,n3);
            for(var i=0; i<count; i++)
            {
                var point = src[i];
                ref readonly var x0 = ref point[n0];
                ref readonly var x1 = ref point[n1];
                dst[i] =  Points.point(x0, x1, f(x0,x1));
            }
            return dst;
        }

        public Points<N3,F> EvalFixedOperator<F>(in ApiMemberCode api, Points<N2,F> src)
