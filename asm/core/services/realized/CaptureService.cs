...
namespace Z0.Asm
{
    ...
    {        
        ...
        [MethodImpl(Inline)]
        static ParsedApiMemory SummarizeParse(in CaptureExchange exchange, in ApiExtractState state, OpIdentity id, ExtractTermCode tc, long start, long end, int delta)
        {
            ...
            return ParsedApiMemory.Define(outcome, bits);
        }

        [MethodImpl(Inline)]
        ParsedApiMemory ParseBuffer(in CaptureExchange exchange, OpIdentity id, ref byte src)
            => capture(exchange, id, (byte*)Unsafe.AsPointer(ref src));

        [MethodImpl(Inline)]
        ParsedApiMemory Parse(in CaptureExchange exchange, OpIdentity id, IntPtr src)        
            => capture(exchange, id, src.ToPointer<byte>());

        [MethodImpl(Inline)]
        ParsedApiMemory capture(in CaptureExchange exchange, OpIdentity id, byte* pSrc)
        {
