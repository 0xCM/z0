...
namespace Z0.Asm
{
    ...
    readonly struct HostCodeArchive : IHostCodeArchive
    {
        ...
        [MethodImpl(Inline)]
        public static IHostCodeArchive Create(IContext context, PartId catalog)
            => new HostCodeArchive(context, catalog);

        [MethodImpl(Inline)]
        public static IHostCodeArchive Create(IContext context, PartId catalog, ApiHostUri host)
            => new HostCodeArchive(context, catalog, host);

        [MethodImpl(Inline)]
        HostCodeArchive(IContext context, PartId catalog, ApiHostUri host)
        {
            this.Context = context;
            this.DefiningPart = catalog;
            this.ApiHost = host;
            this.RootFolder = context.EmissionPaths().DataSubDir(RelativeLocation.Define(DefiningPart.Format(),host.Name));
        }

        HostCodeArchive(IContext context, PartId catalog)
        {
            this.Context = context;
            this.DefiningPart = catalog;
            this.ApiHost = ApiHostUri.Empty;
            this.RootFolder = context.EmissionPaths().DataSubDir(FolderName.Define(DefiningPart.Format()));
        }
