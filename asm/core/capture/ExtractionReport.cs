...
namespace Z0
{
    ...
    using F = ExtractionField;
    using R = ExtractionRecord;

    public enum ExtractionField : ulong
    {
        Sequence = 0 | 10ul << 32,
    ...
    public readonly struct ExtractionRecord : IRecord<F, R>
    {
        public ExtractionRecord(int Sequence, MemoryAddress Address, int Length, OpUri Uri, string OpSig, EncodedData Data)
        {
    ...
    public class ExtractionReport : Report<F,R>
    {        
        public static ExtractionReport Create(ExtractionRecord[] records)
            => new ExtractionReport(records);
        
        ExtractionReport(ExtractionRecord[] records)
            : base(records)
        {
        }        
    }    
